...
type ImageType interface {
	...
	// Returns a serialized osbuild manifest, containing the sources and
	// pipeline necessary to build an image, given output format with all
	// packages and customizations specified in the given blueprint; it also
	// returns any warnings (e.g. deprecation notices) generated by the
	// manifest.
	//
	...
	SManifest(bp *blueprint.Blueprint, options ImageOptions, repos []rpmmd.RepoConfig, resolvers Resolvers, serOptions *manifest.SerializeOptions, seed *int64) (manifest.OSBuildManifest, []string, error)
}
...
type DepsolveFunc func(packageSets map[string][]rpmmd.PackageSet, sbomType sbom.StandardType) (map[string]dnfjson.DepsolveResult, error)

type ContainerResolverFunc func(containerSources map[string][]container.SourceSpec) (map[string][]container.Spec, error)
...
type Resolvers struct {
	// Depsolver resolves dependencies of named packages and generates the full
	// metadata for all packages and their dependencies.
	Depsolver DepsolveFunc

	// ContainerResolver resolves a container ref into the image ID and
	// manifest list digest.
	ContainerResolver ContainerResolverFunc

	// CommitResolver resolves an ostree commit ref and URL into a commit ID
	// (checksum).
