...
type ImageType interface {
	...
	// SerializedManifest returns a serialized osbuild manifest, containing the
	// sources and pipeline necessary to build an image, with all packages and
	// customizations specified in the given blueprint; it also returns any
	// warnings (e.g. deprecation notices) generated by the manifest
	// initialisation.
	//
	// The resolvers argument implements functions for resolving content
	// (packages, containers, ostree commits) from source specifications to
	// fully described content specifications for osbuild.
	// See [Resolvers] for details.
	SerializedManifest(bp *blueprint.Blueprint, options ImageOptions, repos []rpmmd.RepoConfig, resolvers Resolvers, serOptions *manifest.SerializeOptions, seed *int64) (manifest.OSBuildManifest, []string, error)
}
...
// DepsolveFunc implementations should resolve each list of package sets and
// return each merged result in a map with keys corresponding to the input.
type DepsolveFunc func(packageSets map[string][]rpmmd.PackageSet) (map[string]dnfjson.DepsolveResult, error)

// ContainerResolveFunc implementations should resolve each list of container
// refs and return each result in a map with keys corresponding to the input.
type ContainerResolveFunc func(containerSources map[string][]container.SourceSpec) (map[string][]container.Spec, error)

// CommitResolveFunc implementations should resolve each list of ostree refs
// and return each result in a map with keys corresponding to the input.
type CommitResolveFunc func(commitSources map[string][]ostree.SourceSpec) (map[string][]ostree.CommitSpec, error)

// Resolvers defines functions for resolving source specifications into fully
// described, "locked" content specifications (with checksums).
type Resolvers struct {
	// Depsolve resolves dependencies of named packages and generates the full
	// metadata for all packages and their dependencies.
	Depsolve DepsolveFunc

	// ResolveContainers resolves a container ref into the image ID and
	// manifest list digest.
	ResolveContainers ContainerResolveFunc

	// ResolveCommits resolves an ostree commit ref and URL into a commit ID
	// (checksum).
	ResolveCommits CommitResolveFunc
}
